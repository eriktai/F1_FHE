...
int main(void) {
    ...
    std::string working_folder_prefix = "./load_profiling_";

    std::ofstream report_file("load_port_profile.csv");
    report_file << "NumMSHRs";
    for (uint32_t i = 0; i < num_clusters; ++i) {
        report_file << ",Cluster" << i << "_Cycles";
    }
    report_file << ",MaxCycles\n";

    for (uint32_t num_mshrs = 1; num_mshrs <= 8; ++num_mshrs) {
        std::string working_folder = working_folder_prefix + std::to_string(num_mshrs);
        if (fs::create_directories(working_folder)) {
            std::cout << "Created directory: " << working_folder << std::endl;
        } else {
            std::cout << "Failed to create directory: " << working_folder << std::endl;
        }
        std::cout << "Profiling with " << num_mshrs << " MSHRs..." << std::endl;
        ...
        f1::MicroArchConfig micro_arch_config{
            ...
            .cluster_config = {.num_NTT = 1, .num_MM = 2, .num_MA = 2, .num_mshr = num_mshrs,
        ...
        f1::F1ModelResult result = model.compile(dag, root_node, working_folder);

        report_file << num_mshrs;
        uint64_t max_cycles = 0;
        for (uint64_t cycles : result.cluster_total_cycles) {
            report_file << "," << cycles;
            if (cycles > max_cycles) {
                max_cycles = cycles;
            }
        }
        report_file << "," << max_cycles << "\n";

        delete dag; // Clean up DAG for next iteration
    }

    report_file.close();
    std::cout << "Profiling complete. Report saved to load_port_profile.csv" << std::endl;
